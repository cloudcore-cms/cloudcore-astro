// Cloudcore CMS API Client for Astro

const API_BASE = import.meta.env.PUBLIC_CMS_URL || 'http://localhost:8787/api/v1';
...
export async function getCategories(): Promise<Category[]> {
  const result = await fetchApi<{ items: Category[] }>('/public/categories');
  return result.items;
}

// Tags
export async function getTags(): Promise<Tag[]> {
  const result = await fetchApi<{ items: Tag[] }>('/public/tags');
  return result.items;
}
